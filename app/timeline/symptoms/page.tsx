...
import { SymptomsTimelineClient } from './SymptomsTimelineClient'
...
export default async function SymptomsTimelinePage() {
  ...
  const entries = (data ?? []) as SymptomEntryRow[]
    ...
    <main className="flex min-h-screen flex-col bg-surface pb-28">
      ...
      <div className="sticky top-0 z-10 glass-nav border-b border-outline-variant">
        <div className="flex items-center gap-3 px-container-padding-mobile h-16 max-w-lg mx-auto">
          ...
          <h1 className="font-headline font-bold text-headline-md text-on-surface flex-1">Symptoms</h1>
          <Link
            href="/record/symptom"
      ...
      <SymptomsTimelineClient entries={entries} />
      <FAB />
      <BottomNav />
    </main>
  )
