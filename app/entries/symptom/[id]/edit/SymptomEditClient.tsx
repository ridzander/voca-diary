...
import type { SymptomEntryRow, SymptomExtraction } from '@/lib/types'

interface Props {
  entry: SymptomEntryRow
}

export function SymptomEditClient({ entry }: Props) {
  const router = useRouter()
  const [saving, setSaving] = useState(false)

  const handleSave = async (edited: SymptomExtraction) => {
    setSaving(true)
    try {
      const res = await fetch('/api/entries', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: entry.id, mode: 'symptom', data: edited, transcript: entry.transcript }),
      })
      const result = await res.json()
      if (!res.ok) { toast.error(result.error ?? 'Failed to update'); setSaving(false); return }
      toast.success('Entry updated!')
      router.push('/timeline/symptoms')
    } catch {
